...
TaggedLineStringSimplifier::simplify(TaggedLineString* nLine)
{
    ...
    bool simplifyRing = !line->getPreserveEndpoint() && linePts->isRing();
    if (simplifyRing && line->getResultSize() > line->getMinimumSize()) {
        ...
        if (candidateSeg.distance(firstSeg->p0) <= distanceTolerance) {
            if (!hasBadIntersection(line, std::make_pair(0, line->getSegments().size()), candidateSeg)) {
