...
public:
    ...
    void build() {
        ...
        long numChildren = std::distance(begin, nodes.end());

        while (numChildren > 1) {
            createParentNodes(begin, numChildren);
            std::advance(begin, numChildren); // parents just added become children in the next round
            numChildren = std::distance(begin, nodes.end());
        }
...
protected:
    ...
    void createParentNodes(const NodeListIterator& begin, long numChildren) {
        ...
        auto end = std::next(begin, numChildren);
        ...
        for (decltype(numSlices) j = 0; j < numSlices; j++) {
            ...
            end = std::next(begin, numChildren);
    ...
    size_t sliceCount(long numNodes) const {
