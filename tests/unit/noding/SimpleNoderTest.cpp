...
void object::test<1>()
{
    ...
    NodedSegmentString ss1(seq1.release(), false, false, nullptr);

    auto seq2 = std::make_unique<CoordinateSequence>();
    seq2->add(CoordinateXY{1, 0});
    seq2->add(CoordinateXY{0, 1});
    NodedSegmentString ss2(seq2.release(), false, false, nullptr);
